...
fn main() -> std::io::Result<()> {
    ...
    file.write_all(
        "near call gaming.teen_patti2.testnet add_players '{\n \"input_players\":  [\n".as_bytes(),
    )
    .unwrap();
    ...
    for i in 0..num {
        ...
        let card1_string =
            generate_card_string(cards.get(0).expect("Expected a card at index 0"), false);
        let card2_string =
            generate_card_string(cards.get(1).expect("Expected a card at index 1"), false);
        let card3_string =
            generate_card_string(cards.get(2).expect("Expected a card at index 2"), true);
    ...
    file.write_all("  ]\n}' --accountId teen_patti2.testnet".as_bytes())
        .unwrap();

    Command::new("bash")
        .arg("add_players.sh")
        .current_dir(env::current_dir().expect("Can\'t locate current directory"))
        .spawn()
        .expect("No directory for the following name found");
